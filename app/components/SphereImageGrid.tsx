...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const [rotation, setRotation] = useState<RotationState>({ x: 12, y: 18, z: 0 });
  ...
  const actualSphereRadius = sphereRadius ?? containerSize * 0.44;
  ...
  const renderImageNode = useCallback(
    (image: ImageData, index: number) => {
      ...
      return (
        ...
        >
          <div className="sphere-grid__node-frame">
            <img src={image.src} alt={image.alt} className="sphere-grid__node-img" draggable={false} loading={index < 4 ? "eager" : "lazy"} />
          </div>
        </div>
      );
    },
  ...
  if (!isMounted) {
    return (
      <div className="sphere-grid__loading" style={{ width: containerSize, height: containerSize }}>
        Loading partners…
      </div>
    );
  }
  ...
  if (!images.length) {
    return (
      <div className="sphere-grid__empty" style={{ width: containerSize, height: containerSize }}>
  ...
  return (
    <>
      <div
        ref={containerRef}
        className={`sphere-grid ${className}`}
        style={{
          width: containerSize,
        ...
        aria-label="Interactive partner logos. Drag to rotate."
      >
        <div className="sphere-grid__hint" aria-hidden>
          Drag to explore
        </div>
        <div className="sphere-grid__stage">{images.map((image, index) => renderImageNode(image, index))}</div>
      </div>

      {selectedImage ? (
        <div className="sphere-grid__modal" role="dialog" aria-modal="true" onClick={() => setSelectedImage(null)}>
          <div className="sphere-grid__modal-card" onClick={(e) => e.stopPropagation()}>
            <div className="sphere-grid__modal-media">
              <img src={selectedImage.src} alt={selectedImage.alt} className="sphere-grid__modal-img" />
              <button type="button" className="sphere-grid__modal-close" onClick={() => setSelectedImage(null)} aria-label="Close">
                <span aria-hidden>×</span>
              </button>
            </div>
            <div className="sphere-grid__modal-body">
              <h3>{selectedImage.title ?? selectedImage.alt}</h3>
              {selectedImage.href ? (
                <a href={selectedImage.href} target="_blank" rel="noopener noreferrer" className="sphere-grid__modal-link">
                  Visit partner site
                </a>
              ) : null}
            </div>
